data "aws_iam_policy_document" "s3_policy" {
  ...
  statement {
    ...
    resources = ["arn:aws:s3:::tierra-${replace(join("-",list(var.project,element(var.brands,count.index),var.region,var.environment)),"/(.{0,45})(.*)/","$1")}-cloudfront/*"]
...
resource "aws_s3_bucket" "bucket_app" {
  count  = "${length(var.brands)}"
  bucket = "tierra-${replace(join("-",list(var.project,element(var.brands,count.index),var.region,var.environment)),"/(.{0,45})(.*)/","$1")}-cloudfront"
